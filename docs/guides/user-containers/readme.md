...
The token is taken from the workflow specific token that is generated by the NNF software, if
necessary.
...
| Authorization    | Optional | Bearer TOKEN...     | Bearer token for authentication (if token provided)              |
| X-Auth-Type      | Optional | XOAUTH2             | Indicates XOAUTH2 token type (if token is provided)              |
| Accepts-version  | Optional | 1.0                 | API version (required for copy-offload)                          |
